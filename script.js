...
document.querySelectorAll('.video-frame video').forEach(video => { const io = new IntersectionObserver(entries => entries.forEach(entry => { if(entry.isIntersecting){ video.muted=false; video.play().catch(()=>{ video.muted=true; video.play().catch(()=>{}); }); } else video.pause(); }), {threshold:.55}); io.observe(video); });
