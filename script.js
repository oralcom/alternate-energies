...
const cookieManage=document.createElement('button'); cookieManage.type='button'; cookieManage.className='cookie-manage'; cookieManage.textContent='Cookie settings'; cookieManage.addEventListener('click',()=>{localStorage.removeItem('ae-cookie-choice'); if(cookieBanner) cookieBanner.hidden=false;}); document.body.appendChild(cookieManage);
const whatsappNumber='+27635498662'; const whatsappUrl='https://wa.me/27635498662?text='+encodeURIComponent('Hello Alternate Energies, I would like to discuss a solar project.'); const wa=document.createElement('a'); wa.className='whatsapp-float'; wa.href=whatsappUrl; wa.target='_blank'; wa.rel='noopener'; wa.setAttribute('aria-label','Chat with Alternate Energies on WhatsApp'); wa.innerHTML='<svg aria-hidden="true" viewBox="0 0 24 24"><path d="M20.5 3.5A11.8 11.8 0 0 0 12.1 0C5.5 0 .2 5.3.2 11.9c0 2.1.5 4.1 1.6 5.9L.1 24l6.4-1.7a11.9 11.9 0 0 0 5.6 1.4h.1c6.6 0 11.9-5.3 11.9-11.9 0-3.2-1.3-6.1-3.6-8.3ZM12.2 21.7h-.1a9.8 9.8 0 0 1-5-1.4l-.4-.2-3.8 1 1-3.7-.2-.4a9.8 9.8 0 1 1 8.5 4.7Zm5.4-7.4c-.3-.2-1.8-.9-2.1-1-.3-.1-.5-.2-.7.2-.2.3-.8 1-.9 1.2-.2.2-.3.2-.6.1-1.5-.8-2.4-1.4-3.4-3.1-.3-.5.3-.4.9-1.4.1-.2.1-.4 0-.6s-.7-1.7-1-2.3c-.3-.6-.5-.5-.7-.5h-.6c-.2 0-.6.1-.9.4-.3.3-1.2 1.1-1.2 2.7s1.2 3.1 1.4 3.3c.2.2 2.3 3.6 5.7 5 .8.3 1.4.5 1.9.7.8.2 1.5.2 2 .1.6-.1 1.8-.7 2-1.4.3-.7.3-1.3.2-1.4-.1-.2-.3-.3-.6-.4Z"/></svg><b>WhatsApp us</b>'; document.body.appendChild(wa); document.querySelectorAll('.contact-emails').forEach(block=>{if(!block.querySelector('.phone-contact')){const item=document.createElement('div');item.className='phone-contact';item.innerHTML='<span>WhatsApp enquiries</span><a href="'+whatsappUrl+'" target="_blank" rel="noopener">+27 63 549 8662</a>';block.appendChild(item);}});
const contactForm=document.querySelector('#enquiry-form'); if(contactForm && !document.querySelector('#emergency-request')){const emergency=document.createElement('section'); emergency.id='emergency-request'; emergency.className='emergency-request'; emergency.innerHTML='<h2>Emergency maintenance request</h2><p>For existing solar customers who need urgent technical support.</p><form><input required placeholder="Your name"><input required type="email" placeholder="Email address"><input required placeholder="Site location"><textarea required placeholder="Describe the issue"></textarea><button class="button dark" type="submit">Send maintenance request</button><p role="status"></p></form>'; contactForm.parentNode.insertBefore(emergency,contactForm); emergency.querySelector('form').addEventListener('submit',e=>{e.preventDefault();e.currentTarget.querySelector('[role=status]').textContent='Thanks. Your maintenance request is ready for our team.';});}
const emergencyPanel=document.querySelector('#emergency-request'); if(emergencyPanel){const emergencyToggle=document.createElement('button'); emergencyToggle.type='button'; emergencyToggle.className='button lime emergency-toggle'; emergencyToggle.textContent='Emergency maintenance request'; const emailBlock=document.querySelector('.contact-emails'); (emailBlock||contactForm.parentNode).appendChild(emergencyToggle); emergencyPanel.hidden=true; emergencyToggle.addEventListener('click',()=>{emergencyPanel.hidden=false; emergencyPanel.classList.add('is-modal'); emergencyToggle.textContent='Close maintenance request';}); emergencyPanel.addEventListener('click',e=>{if(e.target===emergencyPanel){emergencyPanel.hidden=true; emergencyPanel.classList.remove('is-modal'); emergencyToggle.textContent='Emergency maintenance request';}});}
const qrOpen=document.querySelector('.floating-qr'); if(qrOpen){qrOpen.addEventListener('click',e=>{e.preventDefault();const modal=document.createElement('div');modal.className='qr-modal';modal.innerHTML='<div class="qr-modal-panel"><button class="qr-modal-close" type="button" aria-label="Close">×</button><h2>Company Profile</h2><img src="https://quickchart.io/qr?text=https%3A%2F%2Foralcom.github.io%2Falternate-energies%2Fassets%2Fprofile.pdf%3Fv%3D2%26size%3D320" alt="QR code for Alternate Energies website"><p>Scan to download our company profile.</p></div>';document.body.appendChild(modal);modal.querySelector('.qr-modal-close').onclick=()=>modal.remove();modal.onclick=x=>{if(x.target===modal)modal.remove()};});}
...
document.querySelectorAll('.whatsapp-float svg').forEach(icon=>{const image=document.createElement('img');image.src='assets/whatsapp-icon.png';image.alt='';icon.replaceWith(image);});
